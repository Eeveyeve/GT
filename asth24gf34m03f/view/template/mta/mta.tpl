        ...
        <table class="list">
          ...
          <tbody>            
            ...
            <tr>
							...
							<td class="left">
								<?php if($s['commission_type'] == 'percentage') {?>
								<input type="text" size="20" value="<?php echo $s['signup_code'];?>" readonly="readonly" /><br />
								<small>add </small><strong>&amp;mta=<?php echo $s['signup_code'];?></strong><small> to any shop URL to get new affiliate signed under this scheme</small>
								<?php }?>&nbsp;
							</td>
							<td class="left">
...
$(document).ready(function () { 
	...
	var _cb = function(r) {
		...
		if(r) {
			...
			alert('<?php echo mta_esqq($error_invalid_request);?>');
		}		
	}
	...
	$('#convert_affiliates').click(function() {
		$.get(document.location.href.replace('mta/mta', 'mta/mta/convert_affiliates'), function(r) {
			...
			if(r) {
				...
				$('.success').text('<?php echo mta_esqq($text_convert_success);?>').show();
				$('.warning').hide();
			} else {
				alert('<?php echo mta_esqq($error_invalid_request);?>');
			}
	...
	$('.delete_link').click(function() {
		if(!confirm('<?php echo mta_esqq($text_delete_scheme_warning);?>')) return;
	...
	$('.move_button').click(function() {
		...
		$($(this).parent().find('input:checked')).each(function() {
			if($(this).val() == 'a' && $.inArray(parseInt(_new_id), _all_fixed) != -1) {
				alert('<?php echo mta_esqq($error_fixed_aff_commission);?>');
				return;
			}
