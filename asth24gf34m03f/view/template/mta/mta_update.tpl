...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="view/image/customer.png" alt="" /> <?php echo $heading_title_update; ?></h1>
        ...
        <div id="tab-general">
          ...
          <table class="form">
            <tr>
              ...
              <td><input name="name" value="<?php echo $s_name; ?>" maxlength="100" />
							</td>
            </tr>
            <tr>
              <td><?php echo $entry_description; ?></td>
              <td>
              <textarea name="description" cols="30" rows="3"><?php echo $s_description;?></textarea>
              </td>
            </tr>
            <tr>
              <td><?php echo $entry_is_default; ?></td>
              <td><?php if ($s_is_default) { ?>
                <input type="radio" name="is_default" value="1" checked="checked" />
                <?php echo $text_yes; ?>
            ...
            <tr>
              ...
              <td>
              	<select name="max_levels">              		
              		...
              		<option value="<?php echo $__i;?>"<?php if($__i == $s_max_levels) {?> selected="selected"<?php }?>><?php echo $__i;?></option>
              	<?php }?>
              	</select>	
              </td>
            </tr>              
            <tr>
              ...
              <td>
                <input type="radio" name="commission_type" value="percentage"<?php if($s_commission_type != 'fixed' || $has_affiliates) {?> checked="checked"<?php }?> />
                <?php echo $text_percent; ?>
								<?php if(!$has_affiliates) {?>
                <input type="radio" name="commission_type" value="fixed"<?php if($s_commission_type == 'fixed') {?> checked="checked"<?php }?> />
                <?php echo $text_amount; ?>
								<?php } else {?>
								<br /><span class="help" style="color:#e11"><?php echo $text_no_fixed_for_affs;?></span>
								<?php }?>
							</td>
            </tr>      
            <tr>
              <td><?php echo $entry_before_shipping; ?></td>
              <td><?php if ($s_before_shipping) { ?>
                <input type="radio" name="before_shipping" value="1" checked="checked" />
                <?php echo $text_yes; ?>
                <input type="radio" name="before_shipping" value="0" />
                <?php echo $text_no; ?>
                <?php } else { ?>
                <input type="radio" name="before_shipping" value="1" />
                <?php echo $text_yes; ?>
                <input type="radio" name="before_shipping" value="0" checked="checked" />
                <?php echo $text_no; ?>
                <?php } ?></td>
            </tr> 
            <tr>
              ...
              <td>
              	<select name="eternal">
              		<option value="0"<?php if(!$s_eternal) {?> selected="selected"<?php }?>><?php echo $text_no;?></option>
              		<?php for($__i = 1; $__i <= 50; $__i++) {?>
              		<option value="<?php echo $__i;?>"<?php if($__i == $s_eternal) {?> selected="selected"<?php }?>><?php echo $__i;?></option>
              	<?php }?>
              	</select>	
              </td>
            </tr>
...
$(document).ready(function () { 	
	...
	var _num_levels = get_num_levels();
	...
	var change_tl = function() {
		...
		$('#form_submit_button').click(function() {
			...
			if(_nl < 3 || _nl > 100) return failform('<?php echo mta_esqq($error_name);?>');			
			
			var _cms = $('input[name^="tiers"][type="text"]');
			for(var i = 0;i < _cms.length; i++) {
				if(!(/^\d+(\.\d{1,4})?$/.test($(_cms[i]).val()))) return failform('<?php echo mta_esqq($error_invalid_commission);?>');				
			}			
			
      if($('input[name="commission_type"]:checked').val() == 'percentage') {      	
      	_num_levels = get_num_levels();
      	for(var _i = 1; _i <= _num_levels; _i++) {
      		if(calc_total(_i) > 100 || calc_total(_i) < 0) return failform('<?php echo mta_esqq($entry_tier);?> '+_i+': <?php echo mta_esqq($error_invalid_percent);?>');    	  	
      	}
      }
      
      $.get(document.location.href.replace('update', 'check_scheme_name')+'&name='+encodeURIComponent($('input[name="name"]').val())+'&id='+encodeURIComponent($('input[name="id"]').val()), function(r) {
      	if(!(parseInt(r))) return failform('<?php echo mta_esqq($error_duplicate_name);?>');
      	...
      	jQuery.post(
      		...
      		function(d) {
      			if(d == '1') {
      				document.location.href = document.location.href.replace('/update', '').replace(/\&id\=\d+/, '');
      				return;
      			}
      			return failform(d);
      		},
		...
		var levels_plus = function(_num_levels, _old_num_levels) {
		...
		var levels_minus = function(_num_levels, _old_num_levels) {
		...
		$('select[name="max_levels"]').change(function() {
			...
			if(_num_levels > _old_num_levels) {				
				levels_plus(_num_levels, _old_num_levels);
			} else if(_num_levels < _old_num_levels) {
				levels_minus(_num_levels, _old_num_levels);
			}			
		});
		...
		var hide_percent = function() {
			$('input[name="is_default"]').parent().parent().hide();
			$('input[name="before_shipping"]').parent().parent().hide();								
			$('div[id^="total_div_"]').each(function() {
				$(this).text('$'+$(this).text().replace('%', ''));
			});			
		}			
		...
		var populate_commission = function(c, ad, ap) {
			for(var i = 0; i < c.length; i++) {
				var ip = i + 1;
				for(var i2 = 0; i2 < c[i].length; i2++) {				
					var i2p = i2 + 1;
					$('input[name="tiers'+ip+'_level'+i2p+'_commission"]').val(c[i][i2]);
					if(ad[i][i2]) {
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="0"]').attr('checked', false);
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="1"]').attr('checked', true);
					} else {
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="1"]').attr('checked', false);
						$('input[name="tiers'+ip+'_level'+i2p+'_autoadd"][value="0"]').attr('checked', true);
					}
				}		
				if(ap[i]) {
					$('input[name="level'+ip+'_autoapprove"][value="0"]').attr('checked', false);
					$('input[name="level'+ip+'_autoapprove"][value="1"]').attr('checked', true);
				} else {
					$('input[name="level'+ip+'_autoapprove"][value="1"]').attr('checked', false);
					$('input[name="level'+ip+'_autoapprove"][value="0"]').attr('checked', true);
				}
			}	
		}
		...
		var autoadd = <?php echo $s_autoadd_json;?>;
		var autoapprove = <?php echo $s_autoapprove_json;?>;
		var commissions = <?php echo $s_commissions_json;?>;
		
		levels_plus(_num_levels, 1);
		populate_commission(commissions, autoadd, autoapprove);
		if($('input[name="commission_type"]:checked').val() != 'percentage') hide_percent(); 
		for(var __i = 1; __i <= _num_levels; __i++) {
			set_total(__i);
		}
