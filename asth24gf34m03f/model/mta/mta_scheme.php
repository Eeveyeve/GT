...
class ModelMtaMtaScheme extends Model {
	...
	public function deleteScheme($id) {
		...
		if($fix_default) {
			$res = $this->db->query("select count(*) as c from " . DB_PREFIX . "mta_scheme where commission_type='percentage' and mta_scheme_id!='$id'");
	...
	public function &setScheme($s) {		
		...
		if(!$dsid && !$this->scheme['is_default'] && $this->scheme['commission_type'] == 'percentage') $this->scheme['is_default'] = true;
...
//////////////////////////////////////////////////////////////////////	
	...
	protected function _fixDefaultScheme() {
		...
		if($res->row['c'] > 1) {
			...
			$res = $this->db->query("select min(mta_scheme_id) as id from " . DB_PREFIX . "mta_scheme where commission_type='percentage'");
