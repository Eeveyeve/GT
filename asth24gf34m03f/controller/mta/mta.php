...
'invalid_commission','unknown','database','warning','no_default','no_default_unconverted','all');
	...
	public function settings() {
		if(!isset($this->request->post['setting']) || !is_array($this->request->post['settings']) || !sizeof($this->request->post['settings']) || !$this->user->hasPermission('modify', 'mta/mta')) {
			$this->response->setOutput('0');
			return;
		}	
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('mta_ypx', $this->request->post['setting']);
		$this->response->setOutput('1');	
	}
	*/		
/////////////////////////////////////////////////////////////////
	...
	private function _post_to_scheme(&$p) {
		...
		if($scheme['commission_type'] == 'fixed') $scheme['is_defaut'] = 0;		
