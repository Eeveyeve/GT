...
class ControllerAccountOrder extends Controller {
    ...
    $this->data['text_willbe'] = $this->language->get('text_willbe');
    ...
    $results = $this->model_account_order->getOrders(($page - 1) * 10, 9999);
      ...
      foreach ($results as $result) {
        ...
        if ($first_product['image']) {
  				$first_product_image = $this->model_tool_image->resize($first_product['image'], 60, 75);
  			} else {
  				$first_product_image = '';
  			}*/
        ...
        foreach ($order_products as $product) {
          ...
          if ($productdetail['image']) {
    				$product_image = $this->model_tool_image->resize($productdetail['image'], 88, 125);
    			} else {
    				$product_image = '';
    			}
          ...
          for ($i = 1; $i <= $product['quantity']; $i++) {
            $this->data['products'][] = array(
              ...
              'date_available' => date($this->language->get('date_format_3'), strtotime($productdetail['date_available'])),
              'sys_date_available' => $productdetail['date_available'],
    ...
    if (isset($this->request->get['order_id']) && isset($this->request->get['product_id'])) {
      $key_info = $this->model_account_order->sendOrderKeys($this->request->get['order_id'], $this->request->get['product_id']);      
    } else if (isset($this->request->post['order_id']) && isset($this->request->post['product_id'])) {
      $key_info = $this->model_account_order->sendOrderKeys($this->request->post['order_id'], $this->request->post['product_id']);
    } else {
      $key_info = "";
      $json['error'] = "error";
    }
