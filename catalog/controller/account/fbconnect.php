...
class ControllerAccountFbconnect extends Controller {
	private $error = array();
  	...
  	public function index() {
		...
		if ($this->customer->isLogged() && isset($this->session->data['fb_redirect_url'])) {
      $this->redirect($this->session->data['fb_redirect_url']);
      unset($this->session->data['fb_redirect_url']);
		...
		if($fbuser_profile['id'] && $fbuser_profile['email'] && $fbuser_profile['verified']){
			...
			if($this->customer->login($email, $password)){
				if (isset($this->session->data['fb_redirect_url'])) {
          $this->redirect($this->session->data['fb_redirect_url']);
          unset($this->session->data['fb_redirect_url']);
        } else {
    	    $this->redirect($this->url->link('account/account', '', 'SSL'));
