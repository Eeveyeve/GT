...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
      ...
      if (isset($this->request->post['manual_member_reward'])) {
				$this->session->data['manual_member_reward'] = $this->request->post['manual_member_reward'];
			}
    }  
		// Update
      ...
      if (isset($this->session->data['manual_member_reward']) && $this->session->data['manual_member_reward'] > $this->cart->getSubTotal()) {
      ...
      ? 'https://'
      : 'http://';
      $this->session->data['fb_redirect_url'] = $this->url->link('checkout/cart', '', 'SSL');		
			...
			$this->data['member_reward'] = round(($this->cart->getSubTotal())/100*3, 1);
			...
			if (isset($this->session->data['manual_member_reward'])) {
				...
				$this->session->data['manual_member_reward'] ='';
			}
			
			foreach($total_data as $rekey=>$reval){
				if($reval['code'] == 'total'){
					$total_data[$rekey]['value'] = $total_data[$rekey]['value'] - $this->data['manual_member_reward'];
					$total_data[$rekey]['text'] = $this->currency->format($total_data[$rekey]['value']);
				}
			}
          ...
          $this->data['column_quantity'] = $this->language->get('column_quantity');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/empty_cart.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/error/empty_cart.tpl';
			} else {
				$this->template = 'default/template/error/empty_cart.tpl';
			}
