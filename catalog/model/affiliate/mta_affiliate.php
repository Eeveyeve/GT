...
class ModelAffiliateMtaAffiliate extends ModelAffiliateMtaScheme {
	...
	public function addAffiliate($id, $parent_id, $scheme_signup_code = '', $scheme_id = false) {
		...
		if(isset($this->scheme_id) && $this->scheme_id && $this->scheme && !isset($this->scheme['error']) && $this->scheme['commission_type'] == 'percentage') {
	...
	public function prepareCommissions($aff_id, $aff_info) {
		...
		foreach($aff_info['products'] as $product_id => $_pAr) {
			...
			$res = $this->db->query("select mta_scheme_id, affiliate_id from " . DB_PREFIX . "mta_product_affiliate where affiliate_id in (" . implode(',', $all_aff_ids) . ") and product_id='$product_id' and price_mod_type='$mod_type' and price_mod_id='" . (int) $mod_type_id . "'");
			if($res->num_rows > 0) {
				$_ar = array();
		...
		if(sizeof($not_assigned) > 0) {
			...
			$res = $this->db->query("select mta_scheme_id, affiliate_id from " . DB_PREFIX . "mta_affiliate where affiliate_id in (" . implode(',', $all_aff_ids) . ")");
			if($res->num_rows > 0) {
				$_ar = array();
				foreach($res->rows as $_r) {
					$_ar[intval($_r['affiliate_id'])] = intval($_r['mta_scheme_id']);
		...
		foreach($aff_info['products'] as $product_id => $_pAr) {	
			...
			if($this->scheme['commission_type'] == 'fixed') {
				$aff_data['order_product'][$product_id] = $this->_calculateCommissions($all_aff_ids, null, $_pAr['quantity']);
			} else {
				$_price = $this->scheme['before_shipping'] ? $aff_info['price_before_shipping'] : $aff_info['price'];
				$_price *= $_pAr['share'];
				$aff_data['order_product'][$product_id] = $this->_calculateCommissions($all_aff_ids, $_price);
			}			
		}
	...
	private function _calculateCommissions($aff_ids, $price = null, $quantity = 1) {
		...
		for($i = 0; $i < sizeof($comms); $i++) {			
			$_commission = $this->scheme['commission_type'] == 'fixed' ? $comms[$i] * $quantity  : $price * 0.01 * $comms[$i];
			if($_commission > 0) {
				$out[$aff_ids[$i]] =	array(
	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0) {
      ...
      $_customer_id = $_res->row['customer_id'];
      
      $this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int)$_customer_id . "', order_id = '" . (float)$order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float)$amount . "', date_added = NOW()");
		
    
      $_res = $this->db->query("SELECT customer_id FROM " . DB_PREFIX ."order WHERE order_id = '" . (float)$order_id . "'");
      $_customer_id = $_res->row['customer_id'];
      $_amount = round((($amount/100)*2), 2);
      
      $this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int)$_customer_id . "', order_id = '" . (float)$order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float)$_amount . "', date_added = NOW()");
