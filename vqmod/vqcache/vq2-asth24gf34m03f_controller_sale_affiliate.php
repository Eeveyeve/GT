...
$this->document->addStyle('view/stylesheet/jquery.dataTables.css');//+mod by yp
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateAcccInsert() && $this->validateForm()) {
...
//+mod by yp start	
	private function _get_schemes() {
		...
		$_s = $this->model_mta_mta_scheme->getSchemes(array('fields' => array('mta_scheme_id as id', 'scheme_name as n'), 'filter_commission_type' => 'percentage'));
		$this->_schemes = array();
		foreach($_s as $_s2) {
