...
$this->aff_info['products'][strval($product_id)] = array('mods' => array());//+mod by yp
				...
				if ($product_query->num_rows) {
					...
					$product_discount_query = $this->db->query("SELECT product_discount_id, price FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$customer_group_id . "' AND quantity <= '" . (int)$discount_quantity . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity DESC, priority ASC, price ASC LIMIT 1");
...
$this->aff_info['products'][$product_id]['mods']['discount'] = $product_discount_query->row['product_discount_id'];//+mod by yp
					...
					// Product Specials
					$product_special_query = $this->db->query("SELECT product_special_id, price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$customer_group_id . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1");
...
$this->aff_info['products'][$product_id]['mods']['special'] = $product_special_query->row['product_special_id'];//+mod by yp
            ...
            'genre'           => $product_query->row['genre'],
						'shipping'        => $product_query->row['shipping'],
						'image'           => $product_query->row['image'],
            'manufacturer_id' => $product_query->row['manufacturer_id'],
            'date_available'  => $product_query->row['date_available'],
						'option'          => $option_data,
