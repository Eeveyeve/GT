...
class ModelCatalogProduct extends Model {
    ...
    /*if(!empty($data['filter_special'])) {
      ...
      $sql .= " FROM " . DB_PREFIX . "product p";
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	public function getProductsCatalog($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
    ...
    if (!empty($data['filter_manufacturer_id'])) {
      $conds .= " AND p.manufacturer_id = '" . (int)$data['filter_manufacturer_id'] . "' ";
		}
    
    if (!empty($data['filter_category_id'])) {
      $tables .= " , " . DB_PREFIX . "product_to_category ptc";
      $conds .= " AND ptc.category_id = '" . (int)$data['filter_category_id'] . "' AND ptc.product_id = p.product_id ";
		}
    
    if (!empty($data['filter_special'])) {
			$tables .= " , " . DB_PREFIX . "product_special ps";
      $conds .= " AND ps.product_id = p.product_id AND ps.customer_group_id = 1 ";
		}
    
    if (!empty($data['filter_shown'])) {
      $shown = "AND p.shown != '1'";
    } else {
      $shown = "AND p.shown = '1'";
    }
    
    if (!empty($data['filter_cheap'])) {
      $having .= " HAVING p.price < 20 OR special < 20 OR discount < 20 ";            
    }
    
    if (!empty($data['filter_preorder'])) {
      $conds .= " AND p.date_available >= NOW() ";
    }
    
    if (!empty($data['filter_bestseller'])) {
      $product_order .= " p.viewed DESC ";
    }
    
    /*if (empty($product_order)) {
      $product_order = " p.product_id DESC ";
    }*/
    
    if (empty($product_order)) {
      $product_order .= " p.date_available DESC ";
    }    
    
    $sql = "SELECT DISTINCT *, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special FROM " . DB_PREFIX . "product p, " . DB_PREFIX . "product_to_store p2s, " . DB_PREFIX . "product_description pd " . (string)$tables . " WHERE p.status = '1' " . (string)$shown . " AND pd.product_id=p.product_id AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' " . (string)$conds . " GROUP BY p.product_id " . (string)$having . " ORDER BY " . (string)$product_order . " ";
    ...
    /*$sort_data = array(
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER BY ps.price asc, p.sort_order";	
		}
  ...
  public function getProductSpecials($data = array()) {
    ...
    if (!empty($data['filter_manufacturer_id'])) {
			$filter_manufacturer_id = " AND p.manufacturer_id = '" . (int)$data['filter_manufacturer_id'] . "'";
		}	
				
		$sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' " . (string)$filter_manufacturer_id . " /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.product_id";
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) { 
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getPopularProducts($limit) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed, p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) { 
			...
			$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
	...
	public function getTotalProductSpecials() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' /*AND p.date_available <= NOW()*/ AND p.shown = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))");
  ...
  public function isInCategory($product_id, $category_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (p2c.category_id = c2s.category_id) WHERE p2c.product_id = '" . (int)$product_id . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p2c.category_id = '" . (int)$category_id . "'");
