...
class ModelSaleAffiliate extends Model {
    ...
    $res = $this->db->query("select mta_scheme_id, parent_affiliate_id from " . DB_PREFIX . "mta_affiliate WHERE affiliate_id='$affiliate_id'");
		...
		if($res->row['parent_affiliate_id'] != $data['parent_affiliate_id']) {
			...
			$this->_fix_children($affiliate_id, $parent_ids);
		}         
    return true;
    //+mod by yp end
...
$this->db->query("DELETE FROM " . DB_PREFIX . "mta_affiliate WHERE affiliate_id = '" . (int)$affiliate_id . "'");//+mod by yp
...
'mta_a.level_original'//+mod by yp
	...
	private function _fix_children($parent, $parent_parents) {
		...
		$this->db->query("update " . DB_PREFIX . "mta_affiliate set all_parent_ids='" . $this->db->escape(implode(',', $parent_parents)) . "', level_original='" . (sizeof($parent_parents) + 1) . "' where parent_affiliate_id='$parent'");		
