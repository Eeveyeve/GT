...
$this->data['oy_date_added'] = $this->language->get('oy_date_added');
    ...
    if (isset($this->request->post['shown'])) {
      		$this->data['shown'] = $this->request->post['shown'];
    	} elseif (!empty($product_info)) {
		...
		if($product_image['type_img_video']=='2' || $product_image['type_img_video']=='3')
		$image = $product_image['image'];
		elseif($product_image['image'] && file_exists(DIR_IMAGE . $product_image['image'])) {
				$image = $product_image['image'];
			} else {
				$image = 'no_image.jpg';
			}
			$this->data['product_images'][] = array(
				...
				'type' => $product_image['type_img_video']
			);
		}
